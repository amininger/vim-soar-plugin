...
#### Internal Methods
    ...
    def _create_soar_agent(self):
        if self.settings['source_config'] is not None and self.settings["reconfig_on_launch"]:
            ...
            config_name = self.settings['source_config'].split('/')[-1]
            config_dir = '/'.join(self.settings['source_config'].split('/')[:-1])
            subprocess.check_output(['java', 'edu.umich.rosie.tools.config.RosieAgentConfigurator', config_name], cwd=config_dir)
            self._parse_config_file()
